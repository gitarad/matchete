...
(* Determine Mass substitutions *)
	$Msubs= Flatten@ Table[
		...
		,
		{fieldType,{hScalar, hFermion, hVector, hGhost, hAntiGhost, hGraviton}} (*NB: Loop only over heavy DoFs*)
		];
...
FieldDoFs[f_Symbol, conj_]:= Block[{props= $FieldAssociation[f], inds, i},
	inds= Sequence@@ ConstantArray[i, Length@ props@ Indices + If[$FieldAssociation[f, Type]=== Vector, 1, 0] + If[$FieldAssociation[f, Type]=== Graviton, 2, 0]];
...
ListPowerTypeTraces[order_Integer, lightOnly_:False]:= Module[{possibilities, seed, temp, Xords},
	...
	possibilities= Flatten[Last@ Reap[
		...
		,
			seed= Sow@ {{hScalar}, {hFermion}, {hVector}, {hGhost}, {hAntiGhost}, {hGraviton}};
			While[(Length@ seed> 0),
				temp= Flatten/@ Tuples[{seed, {hScalar, lScalar, hFermion, lFermion, hVector, lVector, hGhost, lGhost, hAntiGhost, lAntiGhost, hGraviton, lGraviton}}];
				seed= Sow@ Select[temp, Total@ BlockMap[Xords, #, 2, 1] < order && Length@ # <= order &];
			];
		];
	], 2];
...
LoopMatch[opt:OptionsPattern[]]? OptionsCheck:= Module[
	...
	logTraces = {hScalar, hFermion, hVector, hGraviton, hGhost, hAntiGhost};
...
fieldFormat= {hScalar-> "\[CapitalPhi]", lScalar-> "\[Phi]", hFermion-> "\[CapitalPsi]", lFermion-> "\[Psi]", hVector-> "V", lVector-> "A", lGhost-> "cA", hGhost-> "cV", lAntiGhost-> "cbarA", hAntiGhost-> "cbarV", hGraviton->"H", lGraviton->"h"};
